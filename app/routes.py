...
logger.info(f"Usando API: {API_BASE_URL}")
logger.info(f"Configurações Redis: URL={REDIS_URL}, HOST={REDIS_HOST}, PORT={REDIS_PORT}")

# Conexão com o Redis
try:
    if REDIS_URL:
        # Tentar conectar usando REDIS_URL
        redis_client = redis.from_url(REDIS_URL, decode_responses=True)
    else:
        # Tentar conectar usando parâmetros individuais
...
def get_resultados():
    cache_key = 'lotofacil:resultados'
    ...
    try:
        # URL fixa para a API da Lotofácil
        api_url = f"{API_BASE_URL}/lotofacil"
        logger.info(f"Buscando dados na API: {api_url}")
